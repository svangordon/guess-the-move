...
class Game(object):
    ...
    def current_analysis(self):
        if self.analysis_results[-1] == 'same':
            ...
            return "Your move:\t{}\tEval: {}\nTheir move:\t{}\tEval: {}\n{}".format(self.user_san(), self.analysis_results[-1][0], self.last_text_move(), self.analysis_results[-1][1], self.get_move_annotation(self.analysis_results[-1][0], self.analysis_results[-1][1]))

    def add_user_move(self, move):
        self.mainline[self.current_turn] = self.mainline[self.current_turn].add_variation(move)
    ...
    def get_input(self):
        if self.current_turn < self.starting_turn or self.current_board().turn != self.user_color:
            self.wait()
        else:
    ...
    def analyse_moves(self):
        ...
        if self._last_user_move is None:
            ...
            if self.same_last_move():
                ...
                user_score = user_move_info['score'].pov(self.user_color).score(mate_score=1000)
                text_score = text_move_info['score'].pov(self.user_color).score(mate_score=1000)
...
if __name__ == "__main__" and False:
    ...
    while not node.is_end():
        ...
        if node.variation(0).move == player_move:
            ...
            messages = [
                "Your move: " + str(player_move) + " Evaluation: " + str(player_move_info['score']),
                "Your move: " + str(node.variation(0).move) + " Evaluation: " + str(text_move_info['score'])
            ]
